...
#' 
#' \code{evolvabilityBeta} calculates (unconditional) evolvability (e), respondability (r), 
#' conditional evolvability (c), autonomy (a) and integration (i) along selection gradients 
#' given an additive-genetic variance matrix as described in Hansen and Houle (2008).
#'  
#' @param G A variance matrix.
#' @param Beta Either a vector or a matrix of unit length selection gradients stacked column wise.
#' @param means An optional vector of trait means (for internal mean standardization).
#' @description \code{G} needs to be symmetric and positive definit.
#' @return An object of \code{class} \code{'evolvabilityBeta'}, which is a list with the following components:
#' \tabular{llllll}{
...
#' \code{c} \tab\tab\tab\tab The conditional evolvability of each selection gradient. \cr
...
#' \code{Minimum} \tab\tab\tab\tab The minimum of the evolvability parameters over all selection gradients. \cr
#' \code{Maximum} \tab\tab\tab\tab The maximum of the evolvability parameters over all selection gradients. 
#' }
#' @author Geir H. Bolstad
...
summary.evolvabilityBeta = function(object, ...) {
    X = list()
    X$call = object$call
    X$Averages = c(e_mean = mean(object$e), r_mean = mean(object$r), c_mean = mean(object$c), 
        a_mean = mean(object$a), i_mean = mean(object$i))
    X$Minimum = c(e_min = min(object$e), r_min = min(object$r), c_min = min(object$c), 
        a_min = min(object$a), i_min = min(object$i))
    X$Maximum = c(e_max = max(object$e), r_max = max(object$r), c_max = max(object$c), 
        a_max = max(object$a), i_max = max(object$i))
