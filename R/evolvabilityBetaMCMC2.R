#' Calculate posterior distribution of evolvability parameters from a selection gradient etimate with uncertainty
#' 
#' \code{evolvabilityBetaMCMC2} calculates (unconditional) evolvability (e), respondability (r), conditional evolvability (c), 
#' autonomy (a) and integration (i) along a selection gradient estimate with uncertainty.
#' 
#' @param G_mcmc A posterior distribution of a variance matrix in the form of a table. Each row in the table must 
#' be one iteration of the posterior distribution (or bootstrap distribution). Each iteration of the matrix must 
#' be on the form as given by \code{c(x)}, where \code{x} is a matrix. A posterior distribution of a matrix in 
#' the slot \code{VCV} of a object of class \code{MCMCglmm} is by default on this form.
#' @param Beta_mcmc A posterior distribution of a unit length selection gradient where iterations are given row wise.
#' @param post.dist logical: should the posterior distribution of the evolvability parameters be saved.
#' \tabular{llllll}{
#' \code{Beta.median} \tab\tab\tab\tab posterior median and highest posterior density interval of the selection gradient. \cr
#' \code{summary} \tab\tab\tab\tab Theposterior median and highest posterior density interval of evolvability parameters. \cr
#' \code{post.dist} \tab\tab\tab\tab The full posterior distributions of the evolvability parameters.
#' }
#' @references Hansen, T. F. & Houle, D. (2008) Measuring and comparing evolvability and constraint in multivariate characters. J. Evol. Biol. 21:1201-1219.
