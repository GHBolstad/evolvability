...
#' 
#' \code{evolvabilityMeans} calculates the average (unconditional) evolvability (e), respondability (r), 
#' conditional evolvability (c), autonomy (a) and integration (i) given the posterior distribution of a 
#' additive-genetic variance matrix using the approximation formulas described in Hansen and Houle (2008, 2009).
#' 
#' @param G_mcmc the posterior distribution of a variance matrix in the form of a table. Each row in the table 
#' must be one iteration of the posterior distribution (or bootstrap distribution). Each iteration of the matrix 
#' must be on the form as given by \code{c(x)}, where \code{x} is a matrix. A posterior distribution of a matrix 
#' in the slot \code{VCV} of a object of class \code{MCMCglmm} is by default on this form.
#' @details The formulas for calculating the evolvability parameters are approximations, except for the formula for 
#' unconditional evolvability which is exact. The bias of the approximations depends on the dimensionality of the G-matrix, 
#' with higher bias for few dimensions (see Hansen and Houle 2008). In most instances we recommend to estimate the 
...
#' }
#' @references 
#' Hansen, T. F. & Houle, D. (2008) Measuring and comparing evolvability and constraint in multivariate characters. J. Evol. Biol. 21:1201-1219. \cr
