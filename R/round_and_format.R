...
#' @param scientific logical: whether encoding should be in scientific notation or not.
#' @param trim logical: if leading blanks for justification to common width should be excluded or not.
...
round_and_format <- function(x, digits = 2, sign_digits = NULL, scientific = FALSE, trim = TRUE){
  if(is.null(sign_digits)){
    format(round(x, digits = digits), nsmall = digits, scientific = scientific, trim = trim)
    } else {
