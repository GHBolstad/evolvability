...
simulate_rate <- function(tree, startv_x = NULL, sigma_x = NULL, a, b, sigma_y = NULL, 
    x = NULL, model = "predictor_BM") {
    
    # include error messages so that ERROR if Trees with negative edge lengths
    ...
    if (model == "predictor_BM" | model == "predictor_gBM") {
        ...
        for (i in 1:nrow(EDGE)) {
            ...
            x_evo[[i]] <- x0 + cumsum(rnorm(n = EDGE[i, 3] * 1000, mean = 0, sd = sqrt(1/1000) * 
                sigma_x))
        ...
        if (model == "predictor_BM") 
            percent_negative <- length(which((a + b * unlist(x_evo)) < 0))/length(unlist(x_evo)) * 
                100
        if (model == "predictor_gBM") 
...
#' @param response The variable for the y-axis of the plot, can be 'rate_y', 'y', or 'x'. 
...
#' @export
plot.simulate_rate = function(x, response = "rate_y", xlab = "Simulation timesteps", 
    ylab = "Evolutionary rate of y", ...) {
    dt <- x$compl_dynamics
    if (is.null(dt$timesteps)) 
        stop("There are no dynamics stored. Plotting is not implemented for the recent_evol model")
    ...
    plot(xvar_list[[1]], yvar_list[[1]], type = "l", ylim = range(unlist(yvar_list)), 
        xlim = range(unlist(xvar_list)), xlab = xlab, ylab = ylab)  #, ...)
