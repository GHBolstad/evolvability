...
for(i in 1:length(b_true)){
  ...
  dt <- try(rate_sim(tree, a = 1, b = b_true[i], x = x, model = "residual_rate"), TRUE)
  mod <- try(rate_gls(x=dt$x, y=dt$y, species=dt$species, tree, model = "residual_rate", maxiter = 100, silent = FALSE), silent = TRUE)
  b_est[i] <- mod$param["b", 1]
}

plot(b_true[1:length(b_est)], b_est)
abline(0,1)
